...
rectangle(X, Y) :- 
    seg(point(X1, Y1), point(X1, Y2)),  % X is a vertical segment
    seg(point(X3, Y3), point(X3, Y4)),  % Y is a vertical segment
    dif(X1, X3),  % Ensure they are different vertical segments
    dif(Y1, Y2),  % Ensure the segments have a height
    dif(Y3, Y4).  % Ensure the segments have a height
    Y1 = Y3,   % Bottom alignment
    Y2 = Y4,   % Top alignment
